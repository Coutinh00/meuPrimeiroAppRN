...
/** Find all JSON schema refs recursively and add them to `refs` Map */ const findRefsRec = (node, refs, path)=>{
    if (node == null || typeof node !== "object") {} else if (Array.isArray(node)) {
        ...
        for(const key in record){
            const value1 = record[key];
            const ref1 = getRef(value1);
            if (ref1) {
                const targetRef1 = parseRefMaybe(ref1);
                if (targetRef1) refs.set([
                    ...path,
                    key
                ], targetRef1);
            } else if (value1 != null && typeof value1 === "object") {
                path.push(key);
                findRefsRec(value1, refs, path);
                path.pop();
            }
        }
    }
...
function jsonSchemaDeref(input) {
    ...
    nextRef: for (const [target, source] of getSortedRefEntries(refs)){
        ...
        for(; targetIndex < target.length - 1; targetIndex++){
            const part1 = target[targetIndex];
            if (outputNode == null || typeof outputNode !== "object" || !(part1 in outputNode)) {
                continue nextRef;
            } else {
                outputNode = outputNode[part1];
            }
        }

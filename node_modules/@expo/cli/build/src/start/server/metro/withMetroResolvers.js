                    ...
                    resolveRequest (ctx, moduleName, platform) {
                        for (const resolver of resolvers){
                            try {
                                ...
                                debug(`Custom resolver threw: ${error.constructor.name}. (module: ${moduleName}, platform: ${platform}, env: ${(ref = ctx.customResolverOptions) == null ? void 0 : ref.environment}, origin: ${ctx.originModulePath})`);
                            }
                        }
...
function withMetroErrorReportingResolver(config) {
    ...
    function mutateResolutionError(error, context, moduleName, platform) {
        ...
        const root = ((ref = config.server) == null ? void 0 : ref.unstable_serverRoot) ?? config.projectRoot;
